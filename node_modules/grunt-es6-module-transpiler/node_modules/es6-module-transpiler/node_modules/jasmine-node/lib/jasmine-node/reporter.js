(function() {
  ...
  jasmineNode.ANSIColors = {
    ...
    skipped: function() { return '\033[33m';  } // Skipped
  };
  ...
  jasmineNode.TerminalReporter.prototype = {
    ...
    reportSpecResults: function(spec) {
      ...
      var msg = '';
      if (result.skipped) {
        msg = this.stringWithColor_('*', this.color_.skipped());
    ...
    printRunnerResults_: function(runner){
      ...
      var specs = runner.specs();
      var msg = '';
      var unskippedSpecs = specs.filter(function(spec){ return !spec.results().skipped; });
      msg += unskippedSpecs.length + ' test' + ((unskippedSpecs.length === 1) ? '' : 's') + ', ';
  ...
  jasmineNode.TerminalVerboseReporter.prototype = {
    ...
    buildMessagesFromResults_: function(messages, results, depth) {
      var element, specResult, specIndentSpaces;
      ...
      for (var i = 0; i < results_length; i++) {
        ...
        if (element.type === 'spec') {
          ...
          var message = this.indentMessage_(element.name, depth), color;
          switch (specResult.result) {
            case 'skipped':
              color = this.color_.skipped();
              break;
            case 'passed':
              color = this.color_.pass();
              break;
            default:
              color = this.color_.fail();
              break;
          }

          messages.push(this.stringWithColor_(message, color));
        } else {
          messages.push('');
          messages.push(this.indentMessage_(element.name, depth));
        }

        this.buildMessagesFromResults_(messages, element.children, depth + 1);
      }
    },
